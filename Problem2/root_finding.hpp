...
double root_simple(
    ...
    int max_steps=10000,     // maximum number of steps to take
    bool root_debug=false)  // print debugging info
{
 ...
 std::cout << "number of steps for simple root finding algorithm = " << step << std::endl;
    return x;
}
    ...
    bool root_debug=false)  // print debugging info
{
    ...
    double f2 = f(x2);
    if (f1 * f2 > 0.0) {
        std::cerr << " root_bisection: f(x1) * f(x2) > 0.0, please provide numbers on which f(x) has opposite signs" << std::endl;
        exit(1);
    }
    ...
    std::cout << "number of steps for bisection algorithm = " << step << std::endl;
    return x_mid;
}
